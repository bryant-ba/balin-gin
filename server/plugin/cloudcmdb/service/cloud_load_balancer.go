...
func (l *CloudLoadBalancerService) List(slb model.LoadBalancer, info cloudcmdbreq.PageInfo, order string, desc bool) (list interface{}, total int64, err error) {
	...
	db := global.GVA_DB.Model(model.LoadBalancer{})
...
func (l *CloudLoadBalancerService) UpdateLoadBalancer(list []model.LoadBalancer) {
	...
	for _, machine := range list {
		...
		if err := tx.Session(&gorm.Session{AllowGlobalUpdate: true}).Clauses(clause.OnConflict{
			...
			global.GVA_LOG.Error("LoadBalancer  messages update fail!", zap.Error(err))
			tx.Rollback()
		}
		...
		if err := tx.Clauses(clause.OnConflict{
			...
			global.GVA_LOG.Error("LoadBalancer messages insert fail!", zap.Error(err))
			tx.Rollback()
		}
...
func (l *CloudLoadBalancerService) AliyunSyncLoadBalancer(cloud model.CloudPlatform) (err error) {
	...
	if err = global.GVA_DB.Where("cloud_platform_id = ?", cloud.ID).Find(&regions).Error; err != nil {
		return err
	}
		...
		go func(region model.CloudRegions) {
			defer func() {
				if err := recover(); err != nil {
					global.GVA_LOG.Error(fmt.Sprintf("aliyun ecs list get fail: %s", err))
				}
			}()
			...
			if err != nil {
				global.GVA_LOG.Error("aliyun LoadBalancer list get fail: ", zap.Error(err))
				return
			}
...
func (l *CloudLoadBalancerService) TencentSyncLoadBalancer(cloud model.CloudPlatform) (err error) {
	...
	for _, region := range regions {
		go func(region model.CloudRegions) {
			defer func() {
				if err := recover(); err != nil {
					global.GVA_LOG.Error(fmt.Sprintf("Tencent slb list get fail: %s", err))
				}
			}()
			...
			list, err := ecs.List(cloud.ID, region, cloud.AccessKeyId, cloud.AccessKeySecret)
			if err != nil {
				global.GVA_LOG.Error("Tencent LoadBalancer list get fail: ", zap.Error(err))
				return
			}
...
func (l *CloudLoadBalancerService) HuaweiSyncLoadBalancer(cloud model.CloudPlatform) (err error) {
	...
	for _, region := range regions {
		go func(region model.CloudRegions) {
			defer func() {
				if err := recover(); err != nil {
					global.GVA_LOG.Error(fmt.Sprintf("huawei ecs list get fail: %s", err))
				}
			}()
			...
			list, err := ecs.List(cloud.ID, region, cloud.AccessKeyId, cloud.AccessKeySecret)
			if err != nil {
				global.GVA_LOG.Error("huawei LoadBalancer list get fail: ", zap.Error(err))
				return
			}
...
func (l *CloudLoadBalancerService) SyncLoadBalancer(id int) (err error) {
	db := global.GVA_DB.Model(model.CloudPlatform{})
...
func (l *CloudLoadBalancerService) LoadBalancerTree(cloud model.CloudPlatform, info request.PageInfo, order string, desc bool) (list interface{}, err error) {
	...
	for _, pt := range platformList {
		...
		if err := global.GVA_DB.Table("cloud_load_balancer").Select("DISTINCT region, region_name").Where("cloud_platform_id = ?", pt.ID).Find(&slblist).Error; err != nil {
			global.GVA_LOG.Error("ecs machine DISTINCT fail!", zap.Error(err))
			return nil, err
		}
