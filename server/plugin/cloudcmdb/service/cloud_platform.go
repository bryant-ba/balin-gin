...
func (p *CloudPlatformService) GetCloudPlatformById(id int) (cloud model.CloudPlatform, regions []model.CloudRegions, err error) {
	if err = global.GVA_DB.Where("id = ?", id).First(&cloud).Error; err != nil {
		return cloud, regions, nil
	}

	if err = global.GVA_DB.Select("id, name").Where("cloud_platform_id = ?", id).Find(&regions).Error; err != nil {
		return cloud, regions, nil
	}
...
func (p *CloudPlatformService) CreateCloudPlatform(cloud model.CloudPlatform) (err error) {
	if !errors.Is(global.GVA_DB.Where("name = ?", cloud.Name).First(&model.CloudPlatform{}).Error, gorm.ErrRecordNotFound) {
		return errors.New("存在相同厂商")
	}
	return global.GVA_DB.Create(&cloud).Error
}
...
func (p *CloudPlatformService) UpdateCloudPlatform(cloud model.CloudPlatform) (err error) {
	return global.GVA_DB.Where("id = ?", cloud.ID).First(&model.CloudPlatform{}).Updates(&cloud).Error
}
...
func (p *CloudPlatformService) DeleteCloudPlatform(req request.GetById) (err error) {
	...
	if err = global.GVA_DB.Where("id = ?", req.ID).First(&cloud).Error; errors.Is(err, gorm.ErrRecordNotFound) {
		return err
	}

	if err = global.GVA_DB.Delete(&cloud).Error; err != nil {
		return err
	}
...
func (p *CloudPlatformService) DeleteCloudPlatformByIds(ids request.IdsReq) (err error) {
	return global.GVA_DB.Delete(&[]model.CloudPlatform{}, "id in ?", ids.Ids).Error
}
